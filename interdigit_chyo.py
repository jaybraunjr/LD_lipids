...
def interdigit(u, nbins=50, nblocks=5, b=0, e=None):
    ...
    names  = groups['chyo'].names #object type
    names  = names.astype(str)    #str type
    resids = groups['chyo'].resids
    ...
    print("Number of chyo atoms: ", groups['chyo'].n_atoms)
    ...
    for ts in u.trajectory[b:e]:
        ...
        dz = bins[1] - bins[0]
        chyo_pos = groups['chyo'].positions
        ...
        d1 = density_frame(groups['chyo'].positions[:,2],
                           groups['chyo'].masses,
                           pbc  = pbc,
                           bins = bins)
        ...
        boolArray  = ((chyo_pos[:,2] > utz) | (chyo_pos[:,2] < ltz)) & np.char.startswith(names, 'O')
        ...
        pp = chyo_pos[boolArray]
        mm = groups['chyo'].masses[boolArray]
    ...
    results['ratio']['chyo-to-pl']      = np.transpose([times, strong_num/numP])
    results['ratio']['chyo-to-pl+chyo'] = np.transpose([times, strong_num/(numP + strong_num)])
    
    results['density'] = {}
    results['density']['PL']        = np.transpose([XX, np.average(d0_densities, axis=0)])
    results['density']['CHYO']      = np.transpose([XX, np.average(d1_densities, axis=0)])
    results['density']['SURF-CHYO'] = np.transpose([XX, np.average(d2_densities, axis=0)])
    results['density']['CORE-CHYO'] = np.transpose([XX, np.average(d3_densities, axis=0)])

    print("units: Z (nm), interdigitation (nm), time (ns), density (g/m3)")
    return results


u = mda.Universe('50.50_4us.part0014.gro', '4us_skip100.xtc')
